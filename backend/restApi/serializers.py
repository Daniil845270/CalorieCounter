...
class FoodDescriptionSerializer(serializers.ModelSerializer):
    ...
    description_owner = serializers.HiddenField(
    ...
    def validate(self, data):
        ...
        if data["prtn100"] > 100 or data["carb100"] > 100 or data["fat100"] > 100:
            raise serializers.ValidationError(
                "100 grams of food can not have more than a 100g of a macronutrient"
            )
        ...
        if data["prtn100"] + data["carb100"] + data["fat100"] > 100:
            raise serializers.ValidationError(
                "Sum of macronutrients per 100g can not 100g"
            )
        ...
        )  # letter, numbers and a whitespace

        for char in submitted_name:
            if char not in allowlist_string:
                raise serializers.ValidationError(
                    "Food item contains illegal characters. Use only letters and numbers"
                )
...
class FoodEntrySerializer(serializers.ModelSerializer):
    ...
    description = serializers.PrimaryKeyRelatedField(
        queryset=FoodDescriptionModel.objects.none
    )
...
class DietStatsSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            ...
            or data["sugar_per_100g"] < 0
            or data["fat_per_100g"] < 0
            or data["food_item_mass_in_grams"] < 0
            or data["kcal_per_100g"] < 0
        ):
            raise serializers.ValidationError("nutrition values can not be negative")

        if (
            data["protein_per_100g"] > 100
            or data["sugar_per_100g"] > 100
            or data["fat_per_100g"] > 100
        ):
            raise serializers.ValidationError(
                "100 grams of food can not have more than a 100g of a macronutrient"
            )
        ...
        # what would be cool to implement is "click the submit button the second time to confirm you are absolutely
        # sure you want to submit this data", like when creating a weak password in fedora account
        if data["food_item_mass_in_grams"] > 5000 or data["kcal_per_100g"] > 900:
            raise serializers.ValidationError("The food mass or kcal value is too big.")

        if (
            data["protein_per_100g"] + data["sugar_per_100g"] + data["fat_per_100g"]
            > 100
        ):
            raise serializers.ValidationError(
                "Sum of macronutrients per 100g can not 100g"
        ...
        if data["entry_date"] - datetime.date.today() > datetime.timedelta(days=6):
            raise serializers.ValidationError(
                "You can not enter items more than a week in a future"
            )
